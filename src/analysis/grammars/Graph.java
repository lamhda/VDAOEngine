...
public class Graph extends Object{
	...
	*/
	...
	public HashMap<String,Vector> outgoingEdges = new HashMap<String,Vector>();
	public HashMap<String,Vector> starCenters = new HashMap<String,Vector>();
	public HashMap<String,Vector> starLeaves = new HashMap<String,Vector>();
          ...
          for(int i=0;i<getNodeNum();i++){
                  ...
                  if(!data.hasGaps)
                    for(int j=0;j<tax.length;j++){
                           d+=VVectorCalc.SquareEuclDistance(data.getVector(tax[j]),getNode(i).x);
                    }
                  else
                    for(int j=0;j<tax.length;j++){
                           d+=VVectorCalc.SquareEuclDistanceGap(data.getVector(tax[j]),getNode(i).x);
                    }
          }
          return Math.sqrt(d/data.pointCount);
      }
	  
	  public double calcFVE(VDataSet data, Vector taxons){
